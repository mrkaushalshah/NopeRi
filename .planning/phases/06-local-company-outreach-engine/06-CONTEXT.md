...
- **Website & Email Scraping:** Assume every valid IT company has a website. Crawl the website (specifically looking for `/careers`, `/jobs`, `/contact` or generic homepage links). Extract ANY email found (e.g., `hr@`, `careers@`, `jobs@`, `info@`, etc.). Strictly DO NOT use Hunter.io or any external email discovery APIs. Rely purely on web scraping the company's own site.
