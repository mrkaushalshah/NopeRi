(function () {
    ...
    function renderWidget() {
        ...
        const recipient = emails.length > 0 ? emails.join(', ') : 'N/A';
    ...
    function executeAutoFill(toEmail, subject, bodyText) {
        ...
        // 2. Fill Subject Field (Bulletproof & dynamic-ID resistant)
        ...
        // 3. Fill Body rich-text contenteditable Editor (Handles Zoho Mail's ze_area iframe)
        let bodyEditor = null;
        let iframe = document.querySelector('iframe.ze_area') || 
                     document.querySelector('iframe[title="Text editor area"]') || 
                     document.querySelector('iframe[aria-label="Text editor area"]');
                     
        if (iframe) {
            try {
                let iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
                bodyEditor = iframeDoc.querySelector('body.ze_body') || 
                             iframeDoc.querySelector('[contenteditable="true"]') ||
                             iframeDoc.body;
            } catch (e) {
                console.error("Could not access iframe document directly:", e);
            }
        }
        
        // Fallback to outer document if iframe wasn't found or accessible
        if (!bodyEditor) {
            bodyEditor = document.querySelector('div[contenteditable="true"].zm-editor-body') ||
        ...
        if (bodyEditor) {
            ...
            // Check if there is already a signature block or text in the editor
            const hasSignature = Array.from(bodyEditor.querySelectorAll('div, span, p')).some(el => 
                el.className.includes('signature') || 
                el.className.includes('zmail_signature') ||
                el.id.includes('signature') ||
                el.innerHTML.includes('Regards') || 
                el.innerHTML.includes('Kaushal')
            );
            
            const existingText = bodyEditor.innerHTML.trim();
            
            if (hasSignature || (existingText && existingText !== '<br>' && existingText !== '<div><br></div>')) {
                // Prepend our cold outreach body at the top, leaving the signature untouched below!
                let container = bodyEditor.ownerDocument.createElement('div');
                container.innerHTML = formattedBody + '<br><br>';
                bodyEditor.insertBefore(container, bodyEditor.firstChild);
            } else {
                // If completely empty, insert normally
